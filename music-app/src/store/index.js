...
const store=new Vuex.Store({
	...
	actions:{
		FETCH_SONG_LIST(context,options){
			...
			p.then(resp=>{
				content.commit("loadSongList",{songs:resp.data.result.songs})
			});
			return p;
		},
		FETCH_SONG(context,music_id){
			...
			p.then(resp=>{
				content.commit("loadSong",{song:resp.data.songs[0]});
			});
			return p;
		},
		FETCH_LYRIC(context,music_id){
			...
			p.then(resp=>{
				content.commit("loadLyric",{lyric:resp.data.lrc.lyric});
			});
			return p;
		},
	},
	mutations:{
		...
		loadLyric(state,payload){
			...
			state.lyricArr=converLrcArr(arr);
